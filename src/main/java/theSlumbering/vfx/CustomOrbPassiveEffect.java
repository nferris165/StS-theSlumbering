...
import static theSlumbering.SlumberingMod.makeVfxPath;
...
public class CustomOrbPassiveEffect extends AbstractGameEffect {
    ...
    private float sX, sY;
    private float tX, tY;
    private static Texture IMG = ImageMaster.loadImage(makeVfxPath("energyRing.png"));
    private Texture img = IMG;

    public CustomOrbPassiveEffect(float x0, float y0) {
        ...
        this.sX = this.x;
        this.sY = this.y;
        this.tX = this.x + MathUtils.random(-16.0f, 16.0f) * Settings.scale;
        this.tY = y0 + MathUtils.random(-16.0f, 16.0f) * Settings.scale;
